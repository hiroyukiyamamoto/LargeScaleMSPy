...
# PCA結果ファイルのパス
pca_output_file = "pca_results.npz"

# PCA結果の読み込み
print(f"Loading PCA results from {pca_output_file}...")
data = np.load(pca_output_file)

# 各データを取得
components = data["components"]  # 主成分
explained_variance_ratio = data["explained_variance_ratio"]  # 分散説明率
pca_scores = data["scores"]  # 主成分スコア
valid_keys = data["valid_keys"]  # 有効なスペクトルキー
...
# %% UMAP 処理
print("Starting UMAP...")
umap = UMAP(n_neighbors=15, min_dist=0.01, n_components=2, random_state=42)
...
umap_df["Spectrum_Key"] = valid_keys  # スペクトルキーを追加
