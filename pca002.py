...
with h5sparse.File(processed_file_path, "r") as f_in, h5sparse.File(filtered_file_path, "w") as f_out:
    all_keys = [key for key in f_in.keys() if key.startswith("spectrum_")]  # スペクトルキーを取得
    total_spectra = len(all_keys)

    for i, key in enumerate(all_keys):
        # スペクトルデータを取得
        sparse_intensity = f_in[f"{key}/intensity"][:]  # スパース行列を取得

        # スペクトルがすべて0の場合はスキップ
        if sparse_intensity.nnz == 0:  # 非ゼロ要素がない場合
            print(f"Skipped {key}: all values are 0")
            continue
...
plt.scatter(umap_results[:, 0], umap_results[:, 1], s=10, alpha=0.8)
